...
export class AIService {
  ...
  async img2txt(imageBase64: string, prompt: string = "Extract all text from this image. Format the output as clean, readable text preserving the original structure and formatting as much as possible."): Promise<string> {
    if (!this.geminiApiKey) {
      throw new Error('Google Gemini API key not configured. Please set VITE_GEMINI_API_KEY in your environment variables.')
    }
  ...
  async analyzeResume(resumeText: string): Promise<AIResponse> {
    const analysisPrompt = `Please analyze this resume and provide a comprehensive evaluation. Structure your response as JSON with the following format:
